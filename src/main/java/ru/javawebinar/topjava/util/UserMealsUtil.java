...
public class UserMealsUtil {
    ...
    public static List<UserMealWithExcess> filteredByCycles(List<UserMeal> meals, LocalTime startTime, LocalTime endTime, int caloriesPerDay) {
        Map<LocalDate, Integer> summaCaloriesToDay = new HashMap<>();
        for (UserMeal userMeal : meals) {
            ...
            summaCaloriesToDay.put(date, summaCaloriesToDay.getOrDefault(date, 0) + userMeal.getCalories());
        }

        List<UserMealWithExcess> list = new ArrayList<>();
        for (UserMeal e : meals) {
            if (TimeUtil.isBetweenHalfOpen(e.getDateTime().toLocalTime(), startTime, endTime)) {
                list.add(new UserMealWithExcess(e.getDateTime(), e.getDescription(), e.getCalories(),
                        (summaCaloriesToDay.get(e.getDateTime().toLocalDate()) > caloriesPerDay)));
            }
        }
        return list;
    }
